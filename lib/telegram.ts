...
export async function handleTelegramUpdate(update: TelegramUpdate) {
  ...
  if (!isTelegramAdmin(actorId)) { await api("sendMessage", { chat_id: chatId, text: "⛔ Bu bot faqat MultBilim administratorlari uchun." }); return; }
  const raw = update.callback_query?.data ?? update.message?.text?.replace(/^\//, "").split("@")[0] ?? "start";
  const command = ["today", "week", "month", "pages", "devices", "briefs", "status"].includes(raw) ? raw : "start";
